...
from backend.common.question_engine.engines.question_transformer import model, tokenizer
...
class TemplateParsingEngine:
    ...
    def generate_questions():
        with open(f"{__location__}/question_bank.json") as question_bank:
            ...
            return sum([TemplateParsingEngine.generate_question_list(question_info) for question_info in data], [])
    ...
    def generate_question_list(question_info):
        ...
        def generate_question(template, category, type):
            try:
                ...
                parsed_answer, parsed_context, answer_value = TemplateParsingEngine.process_template(
...
def update_questions():
    generated_questions = TemplateParsingEngine.generate_questions()
