...
class ShapeQuestionSolver:
    ...
    def __solve_triangle_question(question: TriangleQuestion):
        ...
        if (question.topic == "area"):
            ...
            solution.append(f"To calculate the area of the triangle with side lengths ${a}$, ${b}$, ${c}$, we can use Heron's Formula.")
    ...
    def __solve_rectangle_question(question: RectangleQuestion):
        ...
        if (question.topic == "area"):
            solution.append(f"To calculate the area of the rectangle with with ${width}$ and height ${height}$, we can multiply the width and height.")
    ...
    def __solve_circle_question(question: CircleQuestion):
        ...
        if (question.topic == "area"):
            solution.append(f"To calculate the area of a circle with radius ${radius}$, we can use the formula $A = \pi r^2$, where r is the radius of the circle.")
            solution.append(f"$A = \pi r^2$")
            solution.append(f"$A = \pi \\times {radius}^2$")
            solution.append(f"$A = {circle.calculate_area()}$")
            solution.append(f"Therefore, the area of the circle is ${circle.calculate_area():.3f}$")
        elif (question.topic == "circumference"):
            solution.append(f"To calculate the circumference of a circle with radius ${radius}$, we can use the formula $C = 2 \pi r$, where r is the radius of the circle.")
